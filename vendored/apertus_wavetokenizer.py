...
class WavTokenizer40(WavTokenizerBase):
    ...
    def _load_model(self) -> None:
        ...
        safetensors_path = os.path.join(self.checkpoint, "model-wavtokenizer-model.safetensors")
        ...
        # FLAT_AUDIO_PATCH: flat-to-nested conversion
                    ...
                    "init_args": {
                        "feature_extractor": {
                            ...
                            "init_args": {
                                ...
                                "num_quantizers": 1,
                                "dowmsamples": [6, 5, 5, 4],
                                "vq_bins": config.get("codebook_size", 4096),
                                "vq_kmeans": 200,
                            },
                        },
                        "backbone": {
                            "class_path": "VocosBackbone",
                            "init_args": {
                                "input_channels": config.get("num_filters", 512),
                                "dim": config.get("hidden_size", 768),
                                "intermediate_dim": config.get("decoder_intermediate_size", 2304),
                                "num_layers": config.get("decoder_num_layers", 12),
                                "adanorm_num_embeddings": config.get("adanorm_num_embeddings", 4),
                            },
                        },
                        "head": {
                            "class_path": "ISTFTHead",
        ...
        if os.path.exists(safetensors_path):
            ...
            raise FileNotFoundError(
                    ...
                    "Expected either 'model-wavtokenizer-model.safetensors' or "
                    "'wavtokenizer_large_unify_600_24k.ckpt'.",
                    )

        state_dict = {}
        # BEGIN PATCH 0004: strip audio_tokenizer prefix
        _AUDIO_PREFIX = "model.audio_tokenizer."
        # END PATCH 0004
        for k, v in state_dict_raw.items():
            k = k.removeprefix(_AUDIO_PREFIX) if k.startswith(_AUDIO_PREFIX) else k
            if k.startswith("feature_extractor."):
                if not k.startswith("feature_extractor.encodec.decoder"):
                    state_dict[k] = v

        self.model.load_state_dict(state_dict, strict=False)  # FLAT_AUDIO_PATCH
