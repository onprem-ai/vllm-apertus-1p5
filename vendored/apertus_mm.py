...
#
# Based on vllm-project/vllm PR #50496 (apertus-1-5 branch by Anunay-Yadav,
# built on the GPU-native multimodal refactor from Infomaniak/vllm by blancsw).
#
# Vendored Emu3.5 and WavTokenizer files (apertus_emu35.py,
# apertus_wavetokenizer.py, apertus_utils.py) are used here instead of the
# transformers Apertus1p5VisionTokenizerModel / AutoModel because the
# upstream transformers PR #47662 that adds those classes is not yet merged.
# Once it lands and is released, these vendored files can be removed and the
# imports switched to:
#   from transformers import Apertus1p5VisionTokenizerModel
#   _init_component_model(config.vision_tokenizer_config,
#                         model_cls=Apertus1p5VisionTokenizerModel)
#   _init_component_model(config.audio_tokenizer_config)
...
from .apertus_utils import ApertusImageTokenizer, ApertusAudioTokenizer
...
class Apertus1p5ForConditionalGeneration(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        # Initialize tower placeholder attributes. Actual models are loaded
        # lazily in load_weights() once the model path is known (the upstream
        # PR uses _init_component_model() in __init__, but that requires
        # transformers PR #47662 which is not yet merged -- our vendored
        # tokenizers load from disk instead).
        self.vision_tower: Any | None = None
        self.audio_tower: Any | None = None
        self._image_tokenizer_helper: ApertusImageTokenizer | None = None
        self._audio_tokenizer_helper: ApertusAudioTokenizer | None = None
        self._model_path: str = vllm_config.model_config.model
    ...
    def load_weights(
        ...
        # The vendored GPU-native towers load from their dedicated local files
        # below. Skip their embedded checkpoint tensors on every pipeline rank
        # to avoid loading the same weights again into a different module layout.
        skip_prefixes.extend(["vision_tower.", "audio_tower."])

        # Load tower models from checkpoint disk path using vendored tokenizers.
        if get_pp_group().is_first_rank:
            model_path = self._model_path
            if model_path and self.vision_tower is None:
                image_config = self.config.vision_config
                if isinstance(image_config, PretrainedConfig):
                    image_config = image_config.to_dict()
                self._image_tokenizer_helper = ApertusImageTokenizer(image_config)
                try:
                    target_device = next(self.language_model.parameters()).device
                    target_dtype = next(self.language_model.parameters()).dtype
                except StopIteration:
                    target_device = torch.device("cuda")
                    target_dtype = torch.bfloat16
                logger.info(
                    "[Apertus 1.5] Loading vision tower from %s on %s",
                    model_path, target_device,
                )
                self.vision_tower = self._image_tokenizer_helper.load_vision_tokenizer(
            ...
            if model_path and self.audio_tower is None:
                audio_config = self.config.audio_config
                if isinstance(audio_config, PretrainedConfig):
                    audio_config = audio_config.to_dict()
                self._audio_tokenizer_helper = ApertusAudioTokenizer(audio_config)
                try:
                    target_device = next(self.language_model.parameters()).device
                except StopIteration:
                    target_device = torch.device("cuda")
                logger.info(
                    "[Apertus 1.5] Loading audio tower from %s on %s",
                    model_path, target_device,
                )
                self.audio_tower = self._audio_tokenizer_helper.load_audio_tokenizer(
                    model_path=model_path,
                    device=str(target_device),
    ...
    @staticmethod
    def _extract_vision_code_ids(
        encode_output: tuple[Any, ...],
    ) -> torch.Tensor:
        # Emu3.5 returns (quantized, loss, (metadata, metadata, code_ids)).
        # The language model consumes discrete code IDs, not quantized vectors.
        if (
            len(encode_output) < 3
            or not isinstance(encode_output[2], tuple)
            or len(encode_output[2]) < 3
            or not isinstance(encode_output[2][2], torch.Tensor)
        ):
            raise ValueError("Unexpected Emu3.5 encode output structure")
        return encode_output[2][2]
    ...
    def _encode_image_to_llm_ids(
        ...
        with torch.inference_mode():
            encode_output = vision_tower.encode(image)
            valid_codes = self._extract_vision_code_ids(encode_output).flatten()
    ...
    def _encode_audio_to_llm_ids(
        ...
        with torch.inference_mode():
            # Vendored WavTokenizer uses encode_audio(); upstream transformers PR
            # uses encode() with output.audio_codes (swap when PR #47662 merges).
            audio_input = audio.unsqueeze(0).to(device=target_device, dtype=target_dtype)
            valid_codes = audio_tower.encode_audio(audio_input)
