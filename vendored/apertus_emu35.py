...
def build_vision_tokenizer(
    ...
    safetensors_path = osp.join(model_path, "model-vision_tokenizer-model.safetensors")
    ...
    if osp.exists(safetensors_path):
        ...
        # BEGIN PATCH 0004: strip vision prefix + strict=False
        _PREFIX = "model.vision_tokenizer."
        ckpt = {k.removeprefix(_PREFIX) if k.startswith(_PREFIX) else k: v for k, v in ckpt.items()}
        ...
        raise FileNotFoundError(
            ...
            "Expected either 'model-vision_tokenizer-model.safetensors' or 'model.ckpt'."
        )
